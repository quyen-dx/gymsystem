...
export default function PolicyManagerPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      message.success(t('system_experience.admin.save_success')); setOpen(false); setEditing(null); form.resetFields(); load()
    } catch (error: any) { message.error(error.response?.data?.message || t('system_experience.admin.save_failed')) }
  }
  return (
    <DashboardLayout>
      ...
      <Modal width={760} title={editing ? t('system_experience.admin.edit_policy') : t('system_experience.admin.add_policy')} open={open} onOk={save} onCancel={() => { setOpen(false); setEditing(null); form.resetFields() }}>
        <Form form={form} layout="vertical" initialValues={{ category: 'Chung', isPublished: true }}>
          <Form.Item name="title" label={t('system_experience.admin.title')} rules={[{ required: true }]}><Input /></Form.Item>
          ...
          <Form.Item name="content" label={t('system_experience.admin.content')} rules={[{ required: true }]}><Input.TextArea rows={8} /></Form.Item>

...
export const markPayoutTransferredController = async (req, res, next) => { try { res.json({ success: true, data: await markPayoutTransferred({ payoutRequestId: req.params.id, adminId: req.user._id, transferProof: proofUrl(req) }) }) } catch (error) { next(error) } }
export const resolvePayoutDisputeController = async (req, res, next) => { try { res.json({ success: true, data: await resolvePayoutDispute({ payoutRequestId: req.params.id, adminId: req.user._id, action: req.body?.action, transferProof: proofUrl(req), resolutionNote: req.body?.resolutionNote }) }) } catch (error) { next(error) } }
